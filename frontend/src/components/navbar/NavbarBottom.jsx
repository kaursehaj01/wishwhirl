...
const NavbarBottom = () => {
  ...
  const { setQuery, filteredProducts, query, getTotalCartItem } = useContext(ShopContext);
  const [debouncedQuery, setDebouncedQuery] = useState(query);

  useEffect(() => {
    const handler = setTimeout(() => {
      setQuery(debouncedQuery); // Update the context query
      ...
      <nav className="h-full px-4 max-w-container mx-auto relative">
        <div className="flex items-center justify-between h-full">
                  ...
                  >
                    {/* <Link to={`/product/${product.id}`}> */}
                      <img
                        ...
                        src={product.image || "../../assets/product_13.png"} // Default image fallback
                        alt={product.name}
          ...
          <div className="hidden sm:flex items-center space-x-4">
            ...
            <div className="absolute top-5 right-12 bg-green-600 text-white rounded-full text-xs px-1 py-0">0</div>
...
// const NavbarBottom = () => {
//   const { setQuery, filteredProducts, query } = useContext(ShopContext);

//   const handleSearch = (e) => {
//     setQuery(e.target.value); // Update the search query in context
//   };

//   return (
//     <div className="w-full bg-[#f5f5f3] relative">
//       <div className="max-w-container mx-auto">
//         <div className="flex pt-2 lg:pt-0 flex-col lg:flex-row items-start lg:items-center md:justify-center w-full px-4 pb-4 lg:pb-0 h-full lg:h-24">
//           <div className="relative w-full lg:w-[600px] h-[50px] text-base text-primeColor bg-white flex items-center gap-2 px-6 rounded-xl">
//             <input
...
//                     <div key={product.id || index}  className="max-w-[600px] h-28 bg-gray-100 mb-3 flex items-center gap-3">
//                         <Link to={`/product/${product.id}`}><img onClick={window.scrollTo(0,0)} src={product.image} alt="" className='w-full h-full item-img 2xl:w-full 2xl:h-full'/></Link>
//                       <img className="w-24" src={product.image} alt="productImg" />
//                       <div className="flex flex-col gap-1">
//                         <p className="font-semibold text-lg">{product.name}</p>
//                         <p className="text-xs">{product.new_price}</p>
//                         <p className="text-sm">
//                           new_Price:{" "}
//                           <span className="text-primeColor font-semibold">${product.old_price}</span>
//                         </p>
//                       </div>
//                     </div>
//                   ))}
