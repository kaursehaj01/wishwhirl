...
const Popular = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8090/api/popularinwomen')
      .then((res) => res.json())
...
//   return (
//     <div className="popular pb-[80px] px-4 mx-auto pt-[50px] max-w-2xl sm:px-6 lg:max-w-[85rem]  lg:px-6 ">
//         <h1 className='text-xl sm:text-3xl font-semibold pb-2 justify-center border-b-[1px] flex items-center'>POPULAR IN WOMEN</h1>
//         {/* <hr/> */}
//         <div className=" pt-6 popular-item gap-x-6 gap-y-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8 justify-items-center">
//             {popularProducts.map((item, i) =>{
// return <Item key={i} id={item._id} name ={item.name} image ={item.images[0]} price ={item.price} old_price ={item.old_price}  />
// })}
//         </div>
